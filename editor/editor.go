// based on https://samrapdev.com/capturing-sensitive-input-with-editor-in-golang-from-the-cli/
package editor
...
func CaptureInputFromEditor(resolveEditor PreferredEditorResolver) ([]byte, error) {
	file, err := ioutil.TempFile(os.TempDir(), "journal-*.md") // TODO source file extension from config
	// TODO populate new entry with the template in config
	if err != nil {
		return []byte{}, err
	}
